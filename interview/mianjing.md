...
4. 定位有几种
   5种定位（static、relative、absolute、fixed、sticky）
5. 盒子模型的理解
6. link和@import的区别
...
16. 避免重排重绘的方法
    使用transform和opacity，避免重排重绘，使用gpc在合成阶段处理，脱离文档流操作定位，避免导致其他元素重排
...
12. 不需要调用Object构造函数
13. 字面量创建对象更快，因为不需要查找Object构造函数
14. es6中的weakmap和weakset的作用
15. Generator和async函数的区别
16. bigInt是用来表达大于2^53-1的数
17. promise
18. reflect 重写了一些操作对象的方法
19. === 和 ==，object.is 区别
20. dom事件相关自定义事件， const event = new Event('eventName'), const event = new CustomEvent('eventName', {}) el.dispatchEvent(event)
21. 如何判断数据类型
    typeof instanceof Object.prototype.toString.call() [object Array]
22. valueOf和toString，js隐式转换
23. requestAnimationFrame和 requestIdleCallback区别和使用这个还没看呢
...
28. 使用事件代理有哪些好处
    避免注册多个事件占用过多内存，影响性能
    统一处理代码简洁，新增元素不用在注册事件
29. 进程中堆内存和堆外内存的如何区分
    node 中进程中的内存是堆内存是由v8分配的， 通过流或者buffer操作的数据使用的是堆外内存，流也是通过buffer实现的

30. html文件, css文件, js文件相互影响关系

31. css文件加载（加载就下载和解析）, 不影响html元素的解析, 但是影响render tree 生成, 这也是浏览器自我优化的一部分, 避免多于的重排和重绘,
...
32. 正常不加defer 和 async的script标签, 会同步加载和执行js代码, 所以在加载和执行的时候就会阻止html元素的解析（因为js 可能会改变 DOM 的结构）,
    当添加defer属性时, script标签异步下载js文件, 不会阻塞html元素的解析, 当html元素全部解析完成后, DOMContentLoaed事件触发之前, 按照加载顺序, 执行js文件, DOMContentLoaded事件就是html全部元素都加载和解析完成后, 就会执行, DOMContentLoaded是document上的事件, 当添加async属性时, 会异步下载和执行js代码, 不会阻塞html元素解析, js代码会异步下载完成后立即异步执行, defer和async都是只在外部引用js文件时生效, 正常不加defer 和 async的script标签会阻塞link元素的解析, 所以会阻塞css的加载

33. load事件 是在window上的事件, 是在整个页面的资源加载完成后执行, 就是页面所有资源都下载完成后

34. html元素的解析, 是包括html标签在内的所以元素的解析, 当</html>解析完成后, 才是所有html元素解析完成

35. html和css的加载是并行的，互不干扰的，dom树和cssom树合并形成render树之后就可以开始渲染了，不用解析道最后一个</html>结尾标签，css加载不影响html解析，但是影响dom树的渲染

36. 浏览器在遇到script标签之后如果script标签没有加defer和async，会先渲染一遍之前解析好的html和css,如果有css还没解析好会等css解析好了在渲染，这是为了因为在js中可能获取dom，所以要把之前的都渲染了，以备js中要获取， 然后在去下载和执行js

37. html解析完毕之后就可以通过js获取了，而不用必须渲染完毕

38. css link的话就是现在完毕立即解析， 如果是style标签的话就是执行到理解解析，正常的话css生成和dom解析相匹配的cssom树了就是渲染一下，css和dom就是边解析边渲染的
...
8. 说一下实际使用之后，感觉怎么使用ts比较好
   主要分为三种情况吧
9. 接口相关的每个接口文件对应一套ts类型，参数类型和返回值类型，然后这套类型可以复用，在调用接口的时候也可以使用返回值类型，然后类型定义的时候要写全，要不然没意义了
10. 我正常组件开发中，ts类型定义直接写在组件中就可以了
11. utils方法中也是提出来一个公共的ts文件然后引用， 参数类型和返回值类型
...
使用全局数据时，哪里需要用就在哪里获取，避免上层组件更新带动下层组件跟着更新
...
执行时间过长的时候可能会导致页面卡顿，react16 之前的版本，react 的 vdom 是树形结构，所以当触发更新的时候会进行递归操作，如果递归的层级过深，导致 js 执行时间过长，就会影响页面渲染导致页面出现卡顿，
为了解决这个问题，react16 之后就提出了 fiber 架构，主要就是原来树形结构的 vdom，改为链表结构，在更新的时候时候循环替换取代递归，然后还有时间片和任务优先级的概念，同步任务的优先级会高于异步任务，例如 input 框输入内容，这就是同步任务，同步任务不能被中断，如果是异步任务会进行异步任务调度，每个异步任务有个过期时间，过期时间越小的异步任务优先级越高，时间片就是在浏览器执行完同步任务之后，没有消耗完一帧的时间，剩余的时间可以执行 react 异步任务调度中的任务，根据链表的结构特性如果没有执行完异步任务在剩余的时间中就会保存当前异步任务的，等下次浏览器空闲时继续执行

fiber异步任务分为四类： 紧急异步（拖拽、鼠标滚动）、普通异步（异步请求后的setstate）、延后异步（usetranstion 包裹的state更新）、空闲级异步（永远不超时，组件预渲染等操作）
...
1. 浏览器的“一帧（16.6ms）”流水线步骤
   假设现在新的一帧开始了，浏览器的操作顺序是死磕到底的：

执行普通的 JS 任务： 比如处理点击事件、执行 setTimeout 的回调、处理 Promise 等。
👉 这里就是 requestAnimationFrame 执行的绝对时间点！ 👈
计算样式（Style）： 看看你刚才的 JS 把哪些 CSS 改了。
计算布局（Layout）： 算一算改了 CSS 后，元素的长宽、位置变成了多少。
绘制（Paint）： 拿起画笔，把计算好的像素真正画到屏幕上。
发车完毕，进入空闲等待下一帧： （如果还有剩余时间，这里就是上一题里 requestIdleCallback 干活的地方）。
...
5. react hooks的实现原理
   React使用链表链表将hooks串联起来，不能将hooks放在判断或者循环中是这样不能满足链表的结构要求，导致顺序错乱
...
9. react服务端渲染
   1. 同构、数据注水脱水
      同构就是react代码在服务端运行一遍，然后通过renderToPipeableStream生成html流，返回给浏览器做静态页面展示，然后进行数据注水（就是将数据存在window上），react代码 通过hydateRoot注册之后编译之后在浏览器在运行一遍完成对事件注册，然后完成数据的脱水（将window上的数据，作为初始化数据初始化redux的store），这种react代码在客户端执行一次，在服务端也执行一次的方式就是叫做同构

   看一下初始化state，在useeffect didmount里面是否可以取到？？？？？？， 或者redux初始化在useeffect didmount里面是否可以取到？？？

   初始化state和edux初始化，在useeffect、didmount里面都是可以取到的2. 客户端使用hydrateRoot进行dom事件的绑定3. 服务端使用renderToPipeableStream（node环境用这个）、renderToReadableStream（像deno这种支持web流的，就是ReadableStream和WritableStream这种就要使用）、renderToString（返回字符串）、 renderToStaticMarkup（返回静态页面字符串）、renderToStaticNodeStream（返回静态页面流）， renderToPipeableStream

   react18之前的流方式是 renderToNodeStream 此方法会等待所有 Suspense 边界 完成后才返回输出。

   从 React 18 开始，此方法会缓冲所有输出，因此实际上它并没有提供任何流式传输的好处（这里是啥意思再看看）4. renderToPipeableStream是如何实现的，通过buffer实现的流式传输5. 使用ssr react有哪些注意事项，为什么 useLoayoutEffect react.lazy 不能使用啊 ？？？

   主要是useLoayoutEffect执行顺序顺序问题，会在node环境被执行引发报错？？？
   主要是useLayoutEffect的职责是同步修改dom，在node环境没有dom，水合后操作dom还可能导致页面突然改变导致展示问题6. 使用流展示的页面和正常字符串穿的页面有什么不同呢？？？
   ...
   1. 流ssr，renderToPipeableStream 和 Suspense组件实现流式ssr， 返回html整体结构，然后完成一个Suspense边界就返回这一部分的内容，不用等所有Suspense边界都完成才返回输出，实现增量的页面展示

10. react17和18的新功能 还得看

11. createRoot hydateRoot

12. useTransation 非紧急任务 紧急任务， 原理是什么
...
1. angular 使用真实dom，react和vue使用虚拟dom

1. angular和vue都有指令， react没有指令

1. vue和react都可以实现相同的功能，区别在于数据传递 react单向数据流，vue是双向绑定、模版渲染react是使用jsx、vue是使用模版（vue3也支持jsx了）
...
15. react 合成事件
    react上注册的都是真正的dom事件，而是合成事件，主要是了抹平各个浏览器间的差异，进行事件委托实现合成事件，这样也可以进行性能优化，保证只帮忙一个真实的dom事件

16. 纯函数有啥好处
    输入和输出是确定的，可以做缓存

17. react17之后 react/jsx-runtimne 运行时将jsx转为虚拟dom， 是通过babel编译之后自动引入react-runtime的，不是我们每次开发都引入react了

18. react diff 算法更新机制
...
1. 在逻辑层将虚拟dom传递给渲染层后,将虚拟dom转换为shadow tree

1. yoga将shadow tree中的flex布局转换为原生布局
...
1. jsbridge是异步的，需要序列化反序列化， 通信效率低

1. 还可以通过jsi实现逻辑层和渲染层通信，是同步的，jsi通过c++实现，提高通信效率

1. rn新架构的变化有哪些

1. 使用fabric渲染渲染层, 可以通过jsi实现渲染层和逻辑层通信了

1. turbomodule 实现native模块的按需加载, 在初始化的时候只加载首页需要的native模块,

1. CodeGen 将ts转换为原生代码
...
1. Flutter 在自己的画布上渲染所有组件， React Native 将 JavaScript 组件转换为原生组件。
1. React Native 和 Flutter 都是支持插件开发， React Native 开发的是 npm 插件，而 Flutter 开发的是 pub 插件。
...
9. 混合开发的好处
10. 效率高，一套代码可以配多端, 节省成本
11. 更新方便，不用频繁发版（发到应用市场或者app store），可以热更新
...
13. 选择rn或者是flutter的依据
    选 React Native，如果：

团队全员前端： 你们极其精通 React、TypeScript、CSS，想要最低的转型成本。

Web/移动端一套代码： 你们希望部分代码甚至逻辑能跟现有的 React Web 项目复用。

拥抱 Expo 生态： 现在的 RN 配合 Expo Router 和 EAS 云构建，开发体验极好。

动态化/热更新刚需： 你们极度依赖 CodePush 这种技术，需要随时在后台给用户下发 JS 补丁来修 Bug，而不经过应用商店审核（Flutter 目前官方不推荐且很难做热更新）。

选 Flutter，如果：

极度看重性能与动画： App 里包含大量极其复杂的定制化 UI、图表、绚丽的动画。

团队背景偏客户端/后端： 懂 Java、Kotlin、C# 的人，学 Dart 会觉得极其亲切舒服。

痛恨跨平台 UI 差异： 老板要求双端的设计稿必须 100% 完美复刻，差一个像素都不行。

不想折腾环境和依赖： Flutter 的官方工具链（SDK、调试工具）比 RN 做得更像一个成熟的工业级产品。
...
1. 渲染进程是由主进程创建的，用于渲染窗口页面，每个进程间是独立的
1. 渲染进程和主进程间可以进行ipc通信

1. electron日志上报如何上报

1. 本地日志, 通过ensurefile和appendFile写入本地数据
1. sentry在发生代码错误的时候上传错误

1. electron如何增量更新
1. 使用electron-updater更新
1. 在electron-updater更新的基础上，更新yaml文件，添加stagingPercentage字段，字段的取值是0-100
...
4. 初始化启动electron需要加载大量文件，文件过大导致启动慢，可以拆包避免启动加载不必要的文件

5. 硬件限制，电脑硬件配置不高，导致启动慢，更换性能更好的硬件设备

6. 包体积过大依赖的插件过多导致启动变慢，可以尝试移除不必要的插件
...
4. node如何排查内存泄漏
   场景 A：本地开发或测试环境（最推荐，可视化）
   使用 Node.js 自带的审查工具启动你的程序：
...
点击里面的 Target -> inspect，会弹出一个专门的网页版 DevTools。

切换到 Memory 选项卡。

场景 B：生产线上的紧急排查（代码触发）
在线上你没法开 --inspect 去连浏览器，可以使用 Node.js 内置的 v8 模块，在特定的路由或满足特定条件时，直接在服务器上生成快照文件：
...
6. 如何处理nest多模块依赖
   最佳实践：使用 @nestjs/event-emitter 彻底切断模块依赖
   让模块之间不再直接调用彼此的 Service，而是通过“发布订阅”的事件机制通信。
...
// ...处理下单逻辑
// 下单成功后，向全系统广播，不关心谁来收
this.eventEmitter.emit('order.created', { orderId: 123, userId: 456 });
}
}
短信模块/用户模块：在自己家里监听（订阅事件）
TypeScript
// sms.service.ts
import { OnEvent } from '@nestjs/event-emitter';

export class SmsService {
@OnEvent('order.created') // 听到广播后自动触发，SmsModule 完全不需要 imports OrderModule！
handleOrderCreatedEvent(payload: any) {
console.log(`发送短信通知用户: ${payload.userId}`);
}
}

7. 为什么选择nest，不是express和koa
...
9. 浏览器获取缓存位置的优先级
10. http2 和 http1.1相比 有什么特点
    二进制分帧（就是将传输内容切成二进制的帧碎片，等传输到客户端再进行组装）
    header 压缩（为了减小header大小节省带宽， 实现的方式的Hpack算法，就是使用简短的标识指代复杂的内容，实现映射关系）
    多路复用 （同一个域名就建立一个TCP连接，可以进行多个请求）多路复用因为网络带宽和服务器资源的限制容易导致请求超时， 还有队头阻塞问题，就是同一个TCP连接中的请求一个丢包等待了，其他请求都得等待，不能传输数据了
    服务端推送（服务端推送）

11. http3 实现QUIC协议，依赖udp协议，使用0RTT 快速建立连接，并且QUIC也实现了丢包重发等，弥补了udp的缺点，
    并且实现多路复用同一个连接的多个请求数据传输是独立的，一个请求丢包了等待重发了，其他请求正常传输。

12. 如何实现http2， 实现http2要做哪些事
...
13. 大文件传输方式
14. 分段传输
15. 通过设置 content-type : multipart/form-data, 进行二进制流传输, 然后在使用像 protocal buffer 对二进制进行压缩后在传输

流都是二进制的

11. csrf（跨站请求伪造）
    登录A网站， 然后再去b网站，b网站给a网站后端发请求利用a网站下的cookie信息
    解决方案
1. 设置samesite避免第三方域名携带cookie，
1. 验证refer
1. 使用token 替换 cookie
...
1. 对用户输入进行严格的过滤和转义，我们可以使用动态渲染的方式防范（不能让用户输入的 <script> 变成真正的 HTML 标签）。

2. 开启 CSP（内容安全策略），限制外部脚本的执行。

3. 敏感 Cookie 设置 HttpOnly，让 JS 无法读取。

4. 谨慎使用innerhtml, outerHTML、dangerouslySetInnerHTML，不插入有风险的数据，不直接获取url参数插到html
...
1. 我们可以使用https协议
1. 可以meta禁用iframe <meta http-equiv="X-Frame-Options" content="DENY"> deny 不允许网页中的iframe加载页面， sameorigin 允许相同域名网页被iframe嵌套
1. 获取页面所以iframe标签然后移除
...
向cdn上传资源是上传到源服务器， 有两种更新方式的cdn服务器，一种是主动推动更新的（阿里云就支持主动推送更新，使用的是websorket），一种是使用回源更新（请求源服务端更新缓存的服务器的缓存， 主动刷新缓存、或者缓存预热）
...
6. 如何计算使用那个缓存服务器的IP：CDN根据用户的地理位置分配距离最近的节点

7. ssr渲染使用流传输和正常字符串传输，也面试看到的效果有什么区别，使用流传输有什么好吃

8. 不用http2 和 http3 如何处理并发， 预加载可以用吗
...
1. 本地起一个ws服务，客户端同时监听websocket， 一个客户端请求ws，ws同时向多端发送消息，实现页面间通信， 浏览器对http协议有同源策略，对websocket协议没有
...
1. 首先根据 lightHouse 进行一下性能分析，看一下fcp、lcp、tti这个几个指标，再看一下network请求，看一下js请求的时间和体积大小，看一下图片这类静态资源的请求时间、后端接口的请求时间，分析一下性能问一下原因：

- 发现js加载体积过大加载时间过长，可以对js体积进行优化，可以先使用webpack-bundle-analyzer插件分析包的体积占比，优化体积占比最大的部分（分包将首屏主要逻辑放在一个包、tree shaking、 react.lazy按需加载、 替换掉沉重依赖比如将dayjs替换为momentjs、开启gzip压缩、设置https缓存 强缓存协商缓存）

- 图片过大加载时间长，对图片进行压缩，并上传到 cdn， 使用webp图片

- 接口请求时间长，服务端帮忙优化接口，对接口数据进行缓存（读数据先读缓存，没有再去读数据更新缓存，更新数据库数据后，先删除缓存）

- DNS预解析
  link标签的rel属性设置dns-prefetch，提前获取域名对应的IP地址
- 数据懒加载或者分页，首次返回少量数据，再次请求返回更多数据
- 其他的非首屏优化有react性能优化，和performance记录之后的代码性能点优化
...
12. webp图片如何实现
    前端请求header上带上Accept字段告诉后端是否支持webp，支持就下发webp，不支持就下发png等
...
6. 从缓存的存储位置来说就是两种内存或者磁盘，存储在磁盘的缓存有：servise worker、强缓存和协商缓存、prefetch cache（小的放在内存中，大的放在磁盘中）、localStrorage、cookie
   存在内存中缓存有：cookie（没有设置expires，存在会话内存中）、push cache（存在session内存中， http2的服务端推动的缓存）、 memory cache（请求下来的js 脚本、图片、css样式、不同的浏览器都有自己的设置内存缓存的策略，把比较小的放在内存中，都是通过强缓存协商缓存获取的）
...
1. 语法不同
1. conmonjs是运行时加载，esmodule是编译时加载
1. conmonjs加载的是值的拷贝，内存地址变了，esmodule加载的是值的引用，内存地址相同
...
1. try {} catch {} 运行
2. promise.catch
3. img.onerror script.onerror 和 window.onerror只能获取到代码错误，window.addEventLister('error')可以获取到请求加载错误和代码错误
4. sentry 异常上报，sentry因为可以上传sourcemap，所以可以定位源码
...
1. 搜索速度慢的问题是由于之前搜索的实现是通过将本地sqlite数据库中的笔记全部获取到，然后再通过filter筛选返回结果，这样做在笔记量大时候就非常慢，导致搜索速度慢，一开始测试2000条笔记搜索时间大概为20s

2. 还有笔记时搜索不全的问题，是因为笔记搜索是在每篇笔记的摘要中搜索关键字，而不是在整篇笔记中搜索关键，所以当关键字不在摘要中时，那篇笔记就不能被搜到，需要升级功能实现全内容搜索
...
所以最后选择 sqlite3 + fts5（sqlite3的虚拟表模块，可以实现快速的全局搜索，支持倒排索引） + simple（支持中文分词） 实现全文搜索
...
先拉取数据，有冲突生成一个新的冲突笔记，然后push新的笔记
...
协同算法就是根据用户行为进行分析，然后进行推荐，就是俗话说的大数据，推荐算法

#### 分词如何实现的

分词是将中文或者英文分成单个词，分词的时候会先去预置的字典(字典就是分词的参照和标准)中查找如果有按照字典中的分词，如果没有就按照概率统计、新词发现算法、习惯或者强制分成单字分词，倒排索引是根据分的词和对应的内容id关联，这样搜索一个词就可以根据关联的id搜索到对应的内容
...
我针对这个问题进行调研，发现electron中提供了更新方式，就是通过electron-updater实现更新

找到了方案，通过electron-updater，可以对更新的各个阶段进行处理，然后给出不同的提示文案，下载中、下载完成等，

最后重构了更新流程，实现了检查更新、下载后自动安装这个更新流程

提高了用户体验

并且实现灰度更新，添加stagingPercentage字段，字段的取值是0-100
...
8. 低代码平台的深入架构的什么，数据结构和原理的设计
   本质就是数据驱动视图，json数据控制页面展示

9. ssr用next实现，ssr请求的性能如何优化
   招式一：缓存策略（性能优化的尽头是缓存）
...
1. 对模块组件的开发，开发时要保证兼容pc和h5展示因为h5和pc端的展示效果是不同的、然后都要实现响应式、还有兼容预览和线上的数据、因为为了所见即所得的效果预览的时候也希望有一些交互效果，可以点点击跳转之类的

1. 自定义组件功能，这个里需要支持创建一个空白模块，然后提供集中子元素，让用户取拖动展示，diy一个自己需要模块，这里难点是拖动的一个边界计算，还有拖动过程中的一个顺滑，好有个用户拖动过程中一个预期，还有拖动过程中放手之后的一个定位展示，通过getBoundingClientRect获取位置信息，然后通过grid布局，将空白模块当作一个删格，每次移动都是以删格为基本单位的，中间停止之后根据是否超过半个删格决定是前进一个删格还有后退一个删格，通过这些策论最终实现了自定义模块的功能

1. 还有个就是组件版本控制这里，我们控制了一个版本列表，最多有最近的五个版本，我们开发时候保留五个版本的组件，根据用户选择的版本，做最后的展示
...
我可能会比较看重一是团队的氛围（愉悦、顺畅）、还有一个就是我承担的角色，需要负责的工作（我希望为公司创造更多的价值）、稳定性在这个工作上有一个长期稳定的发展

#### 当遇到意见不同的冲突如何解决

邀请第三人加入，听取意见进行分析，采取当时更加合适的方案

#### 如何设计开发一个公共组件和一个组件库

### 百度的工作总结

1. 负责搜索运营方向业务开发，完成了大事件运营活动（2023春节活动（神龙红包）、 2024春节活动（贪吃蛇红包）、 2025年春节活动（套马红包））等，长线运营活动，搜有红包、答题有用性
2. 行为日志、js异常上报(通过spy进行上报)、接口校验zod，提前发现问题
3. 制定稳定性准入标准，提供降级预案
4. 资源域名被封禁提供备用域名并支持平台自动配置替换
5. 分享降级，修改分享地址配置，度口令降级，更换度口令文案
6. 性能指标：端上屏小于1000ms、tti：<= 2000ms，html体积 <= 14KB，初始css体积 <= 20KB；、js主包体积小于600k、异步分包体积单个小于80k、整体页面节点数小于500
7. 制定核心流程p0case，便于紧急上线快速回归
8. 开发提效aetocss协调UE降低动画开发成本，降低开发成本80%，只剩调试成本
   和ue确定动画导出标准，推动UE使用这个插件产出动画
   遇到的坑点：ue给出动画尺寸过大，导致导致前端定位问题，提前和UE确认动画尺寸，避免后续修改
   aetocss这个插件产出的动画关键帧太多，导致css体积过大，所以我就优化了插件的代码，通过增加关键帧之间的时间减少关键帧的代码，在保证效果的前提下，优化了30%的体积
9. 给项目做的公共优化
10. 在同步加载的js体积过大时，进行异步加载分包

11. 印象深刻的项目
    套马红包：
    背景：想结合马年这个主题做个有交互性的套马活动，在此为基础上开始进行调研，这个调研过程中调研了three.js和Galacean Effects（是蚂蚁出的移动端动效方案），通过写demo发现，threejs上手成本高，和UE的调试成本也很高，对我们期望快速上线，轻量化的随求不相符，Galacean Effects，这是一种动效播放模式，将Galacean Effects编辑器产出的物料进行播放，不需要前端去操作额外的交互，（还有一种模式是是gelance Engine可以实现复杂的3d交互（遵循 ECS（实体-组件-系统）3D互动引擎）），我通过demo，发现Effects模式就是可以满足我们的想要的效果，主要依赖UE的成本产出物料，开发成本相对较低，上手较快，且经历了蚂蚁的验证，兼容可以得到保障，所以最终选择了Galacean Effects进行套马活动的开发
...
1. 设计师产出的物料设置降级帧，在代码中使用降级插件@galacean/effects-plugin-downgrade'，并在loadscene中设置降级参数

如何判断套中：
这是一个AABB矩形碰撞检测，绳索的坐标是x,y, 只要绳索的坐标是在马的身体中心向左或向右1.24，向上或向下0.7这个范围内就表示套中了，使用矩形碰撞检测的原因是计算成本低，性能高，一开始调这个套中马这个坐标花了一些时间，一开始不是偏左就是偏右，后面才确定了整体的矩形范围
...
1. 单合成播放情况时，Player 会自动销毁之前的合成，如果多合成播放，请手动销毁不需要的合成 composition.dispose()，避免内存叠加。
   1. 动画合成的情况需要手动控制纹理的释放

1. 复杂的多页面情况，页面如果压栈，请暂停全部的播放器，页面回前台的时候再恢复，如果后续页面内存压力大，可以卸载纹理释放内存。
   1. 离开页面需要暂停动画并且释放纹理 player.pause({offloadTexture:true})
1. 一旦页面同时存在的 Player 超过 8 个，Chrome 会自动销毁之前创建的 Player。
1. CSS 动画不适合和大面积 Canvas 动画一起播放。
1. 剪裁图片，移除重复图片，尽量减少图片尺寸大小。
1. 网络条件允许的情况下使用压缩纹理, 最新版运行时已经支持KTX2纹理, 需要用到大纹理(2K)以上的建议使用。
...
面试官您好，我叫张横，目前在百度搜索担任前端开发工程师，主要负责搜索运营增长相关的项目（包括大事件项目、长线活动项目），在此之前，在网易和高途教育等公司任职，在网易主要负责有道云笔记桌面开发、网易外贸通建站项目的开发，我主要的技术栈是React。

### 低绩效的原因

我上一年绩效是M-, 我和我的老板深入沟通后，复盘之后，我确实有做的不足的地方，作为项目的负责人， 端到端的业务思维有所欠缺，对复杂项目推进节奏不足。

基于这几个问题我也详细反思了一下，
首先是端到端的业务思维欠缺，比如做的悬浮球的技术需求，虽然转化率提升了，但是只盯了业务指标，没有前置评估对搜索大盘的性能影响，导致悬浮球没有及时上线，后续补了性能数据之后才进行上线，还有搜有红包的功能上线后，功能虽然做完了，但是对转化率数据分析不够敏锐，没有及时进行数据分析，让我深刻理解了不能只管将代码上线就完事了，要对最终业务效果和系统大盘负责，后续的需求我们及时进行数据分析，和pm及时沟通探讨优化方法

然后是项目质量把控不严问题：让理解了不能为了赶进度而忽视项目质量，后续将在提测前，梳理自测list，充分自测后再进行提测，保证项目质量，还有就是复杂项目的推进节奏不足的问题，这里暴露了我向上管理和跨部门推进的问题，后续我会指定明确的汇报频率，及时同步进度和暴露问题寻求帮助解决问题，在跨部门推进时要明确影响，表明这个是大家共同目标，有利于问题的解决和推荐

这次M-对我来说是一个痛苦的成长，也希望成长后的我能来带咱们团队，在技术特点和咱们团队这么契合的情况下，我有信心能打一个漂亮的翻身仗。

### 为什么要trans

我在搜索运营工作的这段时间也成长了很多，积累了激励相关的经验、还有和端、任务中台的交互也积累了经验，团队的氛围也很好，我看咱们团队的业务方向，是ai的方向，可以快速生成页面、小程序，这个我非常感兴趣，也充满了挑战，我以后想往这个方向发展，还有咱们团队的技术栈和我技术栈非常契合，我来到咱们团队之后可以快速上手工作。

今天晚上和hr聊了一下，前面没有什么问题，最后卡在了我去年绩效是M-这里，我还是想要和咱们争取一下这个岗位，我们运营这边因为业务合并PM团队调整，没有强产出，导致部分需求上线后数据确实没有达到预期，关于M-老板评价的核心问题我也进行了复盘

1. 千行bug率不达标：我的bug率为：我去年一整年的千行代码bug率为5.7，因为运营这边需要和端合作、测试也分前后端，所以所有的bug都会先提到前端，虽然把bug转出去了但也会统计上，我现在会在提测进行个人的Checklist自测，这种梳理是存在思维盲区的，后续工作流应该增加了更严格的边界用例分析和自动化扫描辅助，用系统性的手段来拦截风险，进一步确保交付质量的底线。
2. cr数量不足：去年客观上我所在方向的架构设计导致能参与的 CR 权限较少，大部分收口在方向负责人处，但我应该更积极的去请求参与 CR， 提一些非业务上的问题。
3. 端到端的业务思维欠缺：
   3.1 搜有红包的功能上线后，功能虽然做完了，但是对转化率数据分析不够敏锐，没有及时进行数据分析。我意识到了这个整体思维问题，我现在已经把“看数据”加入了我的标准工作流中，不仅盯技术指标，更会主动和业务侧探讨下一步的增长优化方案。
   3.2 悬浮球的技术需求：虽然转化率提升了，但是只盯了业务指标，没有前置评估对搜索大盘的性能影响，后面才补了性能数据，我后续会在技术设计阶段就应该加入更全面指标的判断和设计。
4. 复杂项目的推进节奏不足的问题：针对于跨部门推进进度欠缺，难以快速推进问题，我建立了一套“风险暴露机制”。遇到推进难点，我会第一时间向上汇报，但不是只抛问题，而是带着“备选方案”去寻求资源协调和帮助，确保整体节奏可控。
   我非常看好咱们部门的业务方向，我过往的经验也与咱们的需求高度契合，我通过之前“踩的坑”现在可以更好的完成工作，在这个新岗位上快速进入状态，我希望可能的话还有机会加入到咱们的团队

### 前端架构设计

1. 工程化与技术选型（地基维度）
   技术选型决定了项目的上限和未来的生态维护难度。

仓库管理模式： 评估采用 Monorepo（单仓多包，适合大型复杂项目或组件库、跨端应用）还是 Polyrepo（单项目单仓）。

构建工具链： 根据项目规模选择现代构建工具（如 Vite 追求开发体验，Rsbuild / Rspack 追求极致打包速度，Webpack 处理极复杂的陈旧项目）。

包管理器： 统一使用 pnpm（速度快、省空间、避免幽灵依赖）或 Bun。

开发语言： 商业项目闭眼选 TypeScript，并严格制定 tsconfig 规则，禁止滥用 any。
...
2. 代码架构与分层设计（核心维度）
   这是架构设计中最具含金量的一环。必须贯彻“关注点分离（Separation of Concerns）”原则，严禁把网络请求、数据处理、UI渲染全部塞进一个文件里。

推荐的企业级四层架构设计：

视图层（View）： 纯粹的 UI 组件（React/Vue）。只负责接收 Props、渲染界面、触发事件。组件设计遵循原子化设计（基础组件、业务组件、区块、页面）。

状态层（Store/State）： 区分全局状态（用户信息、权限）、本地状态（组件内部控制弹窗）、以及服务端状态（如用 React Query / SWR 管理的接口缓存数据）。

服务层（Services/API）： 统一封装 Axios / Fetch，定义拦截器，将 API 请求按模块抽离，并做好 TS 类型声明（DTO/VO）。

业务逻辑/领域层（Utils/Hooks）： 把复杂的计算逻辑、表单校验、格式化函数抽离到纯 JS/TS 函数或自定义 Hooks 中，确保逻辑可以跨组件复用甚至脱离框架运行。

3. 数据流与状态管理（血液维度）
   不合理的全局状态管理是前端项目后期卡顿和 Bug 频发的根源。

状态归属权： 坚守“状态能不全局就不全局”的原则。优先使用组件本地状态。

选型匹配： \* 中轻量级项目、多团队协同：推荐 Zustand（React）或 Pinia（Vue），上手快、心智负担小。

极巨型、数据关系错综复杂：考虑 Redux Toolkit（严格的单向数据流控制）。

异步数据隔离： 强烈建议将“接口请求回来的数据”与“纯前端的交互状态”分离，使用数据响应式缓存库（如 TanStack Query）管理后端数据。

4. 规范与质量保障（防腐维度）
   人人都写出风格统一的代码，架构才不会在团队扩张中“腐烂”。

代码规范（Lint/Format）： 接入 ESLint + Prettier（或现代全能工具 Biome），并强制开启保存自动格式化。

提交拦截（Git Hooks）： 利用 Husky + lint-staged。在用户执行 git commit 的瞬间，自动跑代码校验和格式化。如果校验不通过，直接拒绝提交，从源头杜绝脏代码进入代码仓。

测试策略：

核心公共库/Utils： 强制通过 Vitest / Jest 进行单元测试。

核心主业务流程： 引入 Playwright / Cypress 做自动化端到端（E2E）测试，保障核心冒烟测试通过。

5. 稳定性与可观测性（监控维度）
   这是很多初级架构师容易遗漏、但大厂极为看重的一点——“应用在线上裸奔是极其危险的”。

全局错误捕获： 接入 Sentry 或自研监控平台。捕获 JS 运行时错误、Promise 未处理拒绝、资源加载失败等。

性能指标追踪（Web Vitals）： 收集并统计真实用户的 LCP、INP、CLS 等核心用户体验指标，通过数据看板驱动后续的性能劣化治理。

用户行为回溯： 记录错误发生前的用户点击轨迹（Breadcrumbs），极大提升线上盲盒 Bug 的排查效率。

### ai写代码如何写提示词

企业级“黄金提示词模板”
当你需要 AI 写组件时，可以直接套用下面这个结构填空：

角色： 你是一位资深的前端架构师。

任务： 请帮我用 [React 18 + TypeScript + Tailwind CSS] 开发一个 [带自适应折叠的多选标签组] 组件。

详细要求：

功能与交互： > - 支持点击选中/取消选中标签，支持多选。

当标签数量超过一行时，末尾自动显示“展开/收起”按钮，点击可切换展示全部。

需要处理 loading 状态（展示骨架屏）和 disabled 状态。

Props 接口设计 (TypeScript)：

options: 标签数据数组，结构为 { label: string, value: string | number }

value: 当前选中的值数组。

onChange: 选中状态改变时的回调函数，参数为当前选中的 value[]。

样式规范：

整体风格偏向极简，默认标签为灰色背景，选中状态为系统主题色（bg-blue-600 text-white）。

必须具备良好的响应式，在移动端单行横向滚动，在桌面端自动换行。

代码质量：

请给出完整的、模块化的代码，不要有任何省略。

核心函数和属性必须附带 JSDoc 注释，禁止引入任何未说明的第三方依赖。

### 如何训练一个模型

### cloade 如何节约token和答案更准确

### 如何开发一个mcp，使用node， 开发了哪些mcp

开发了调用接口平台mcp根据，描述自动获取接口平台地址调用测试接口

### 如何开发agent， 开发了什么agent

开发了搜索内部文档的agent，可以快速定位部门内部文档，并且有本次存储可以存储用户的自定义喜好

### 如何压缩token

一、 核心杀手锏：配置 .claudeignore（源头切断）
这是最立竿见影的手段。Claude Code 在启动时会扫描项目目录，如果没有配置忽略文件，它可能会把一些巨大的文本文件、依赖包、锁文件悄悄读进上下文。

在项目根目录下创建一个 .claudeignore 文件，把所有不需要 AI 阅读的“Token 刺客”全部挡在外面：
...
# 阻断大文件和锁文件（极其消耗 Token 且 AI 不需要看明细）
...
.cache/
.git/
🧹 二、 善用 /compact 命令与定期重启（清理短期记忆）
Claude Code 是一个交互式的 CLI，你和它的每一轮对话、它执行的每一次终端命令输出、每一次报错日志，都会叠加存入当前会话的上下文历史中。对话越长，后面每一次发送请求的 Token 就会成倍暴增。

使用 /compact 命令：
当你在终端发现响应变慢，或者你已经在一个会话里跟它聊了很久，直接输入 /compact。Claude Code 会启动一个后台任务，把你们之前的长篇大论压缩浓缩成一份精简的简报（Summary），然后清空冗余的历史日志。这样可以瞬间腾出大量的 Token 空间。

见好就收，随时重新开启会话：
如果一个 Bug 已经修完了，准备干下一件毫不相干的事，千万不要在当前的窗口继续聊。直接退出，然后重新输入 claude 开启一个干净的全新会话。

🧠 三、 移库移库：用 Skills 系统替代臃肿的 CLAUDE.md
很多团队习惯在项目根目录下写一个巨大的 CLAUDE.md 文件，里面塞满了密密麻麻的团队规范、Java/JS 编写指南、部署命令。

问题： CLAUDE.md 是 Claude Code 的全局最高常驻上下文。这意味着只要它存在，Claude 跟你说任何一句话，都要把这个几千字的 Markdown 文件全量带上。

压缩解法： 把长篇大论的规范打散，做成 Skills（存放在 .claude/skills/ 下）。
正如之前提到的，Skills 支持延迟加载（Lazy Loading）。你可以把前端规范写在 frontend.md 里，并加上触发器。这样只有当 Claude 去改前端文件时，这部分 Token 才会被加载，平时完全不占空间。

🔍 四、 控制工具的“输出量”（防止终端大文本轰炸）
Claude Code 拥有执行 Bash 的能力。如果它执行了一个命令，终端吐出了几万行日志，这些日志会一字不落地全部变成 Token 塞进它的脑子里。

教导它使用精确的命令：
如果你发现它准备执行全局搜索或打印大日志，你可以主动干预它。

❌ 坏示范：让它执行 cat huge-file.json。

✅ 好示范：提醒它：“请使用 grep 或 head -n 20 只查看该文件的前 20 行，不要全量打印。”

利用 PreToolUse Hook 强行截断输出：
你可以在我们前面聊到的 .claude/settings.json 的 Hooks 里，对频繁吐出大文本的命令做一层包装，或者给它的 Bash 工具加上输出行数限制（超过 1000 行自动截断），从而在底层保护你的 Token 钱包。

### skill的好处和延迟加载如何实现

一、 使用 Skill 的核心好处

1. 极致的 Token 经济性（省钱、防爆）
   如果把所有规范（前端、后端、测试、部署、Git）都塞进 CLAUDE.md，Claude 每回答一句话都需要全量阅读这些内容，基础底噪 Token 极高。而使用 Skill 后，知识被离散化，只有在特定场景下才会加载，日常对话的 Token 消耗可以降低 60% ~ 80%。

2. 突破上下文窗口限制（不健忘）
   大模型的长上下文（如 200K）虽然很大，但内容越多，AI 的“注意力”越容易分散。将任务打包成 Skill，可以保证 AI 在执行特定任务时，上下文中只有最纯净、最相关的指导原则，从而大幅提升 AI 编写复杂代码的精准度。

3. 团队知识的“模块化”与可复用性
   Skill 以独立的 Markdown 文件形式存在于 .claude/skills/ 目录下。团队成员可以像管理 npm 包一样管理 AI 技能：

资深架构师写一个 db-migration.md 规范。

前端组长写一个 tailwind-style.md 规范。

新人拉取代码后，开箱即用，AI 自动具备全套专家级视野。

4. 显式命令支持（打造专属工具箱）
   Skill 可以直接映射为 CLI 的斜杠命令（如 /review、/i18n）。这让开发者可以把 AI 当成一个高效率的脚本执行器，精确指引它去干某件特定的体力活。

Skills 的“延迟加载”办法
主厨上班时，你只给他一张 1 页纸的“菜谱目录”。
这张纸上只写了：

如果有人点【西餐/以 .tsx 结尾的菜】，请去 A 抽屉拿《西餐标准规范》。

如果有人点【中餐/以 .java 结尾的菜】，请去 B 抽屉拿《中餐标准规范》。

平时主厨怀里只有这一页纸，轻装上阵（日常对话极省 Token）。

🛠️ 底层真实的 3 步执行过程
回到代码里，当你对 Claude code 说：“帮我改一下 UserCard.tsx 这个前端文件” 时，底层发生了下面三件事：

第一步：对暗号（触发器命中）
Claude code 这个客户端程序（注意：是运行在你电脑上的软件，不是 AI 大脑）拿到了你这句话，发现你提到了 UserCard.tsx。
它立刻去翻那张“菜单目录”（Skill 文件顶部的元数据），发现：

YAML
triggers:
globs: ["**/*.tsx"] # 只要是 tsx 文件，就触发我！
触发行动！ 此时系统知道：需要加载这个前端 Skill 了。

第二步：悄悄“塞小抄”（动态注入）
在把你的请求发送给远端 Claude 大脑之前，本地程序在后台偷偷做了一个拼接。它把这个 Skill 文件里写满了几千字的前端规范，硬生生变成了一段临时提示词（小抄），塞进了这次请求的屁股后面。

最终发送给 AI 大脑的数据变成了：

人类的请求： 帮我改一下 UserCard.tsx。
本地程序偷偷加的系统指令（小抄）： 顺便报告大脑，你现在正在处理前端文件，请立刻严格遵守以下规范：[此处注入几千字的前端样式和架构规范...]

第三步：用完即焚（释放 Token）
AI 大脑看完了“小抄”，非常完美地帮你把 UserCard.tsx 修改好了。
当你开启下一个任务，比如问它：“今天周几？”或者让它去改一个 db.sql 后端文件时，刚才塞进去的几千字前端规范会被本地程序彻底从发送队列里抹去。

📌 最后的总结
所谓的延迟加载（Lazy Loading），并不是大模型自己学会了“按需看书”，而是你电脑上的 Claude Code 客户端充当了“书童”。

书童平时不给大模型看厚书，而是死死盯着大模型的操作。一旦发现大模型要碰某种特定的文件，书童就临时把对应的菜谱（Skill 规范）拍在大模型的桌子上；大模型用完，书童立刻把书收走。
