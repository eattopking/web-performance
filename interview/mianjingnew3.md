...
1. 客户端请求 ➡️ 2. Middleware (中间件)
   大楼角色： 大楼正门口的“红外线扫描仪”和“传达室”。

核心作用： 它是请求进入框架后的第一道防线。它拥有最高权限去读取和修改请求对象（Req）和响应对象（Res）。

常见任务： \* 打印访问日志（谁在什么时间访问了）。

开启跨域支持（CORS）。

解析请求体（把用户传来的加密文本变成 JSON）。

3. Guard (守卫)
   大楼角色： 电梯口的“工牌刷卡机”或“保安”。

核心作用： 做权限拦截。 守卫只关心一件事：“你有没有资格进这栋大楼？”（canActivate）。如果没带工牌或权限不足，直接原地劝退，返回 403 Forbidden。

常见任务： \* 检查用户是否登录（JWT 登录态校验）。

检查用户角色（比如：这个普通员工想访问“财务管理”页面，直接拦截）。

4. Interceptor (拦截器，进入路由前)
   大楼角色： 前台接待员，在你填表前给你启动一个“业务计时器”。

核心作用： 拦截器可以横跨“请求前”和“响应后”。在进入路由前，它可以用来绑定额外的逻辑、拦截并改变数据流。

常见任务： \* 记录该业务是从哪一毫秒开始处理的，用来计算接口耗时。

缓存拦截（如果发现上一个客户刚查过一模一样的数据，直接把缓存拿出来，不让客户往里走了）。

5. Pipe (管道)
   大楼角色： 前台的“资料审核窗口”。

核心作用： 做参数校验与类型转换。 客户提交上来的表单数据可能是乱七八糟的，管道负责在送给经理前把数据理干净。

常见任务： \* 类型转换： 网页传过来的数字通常是字符串 "18"，管道自动把它变成真正的数字 18。

数据校验： 检查邮箱格式对不对、密码长度是不是大于6位。如果校验失败（比如年龄写了负数），直接在这里报错拒绝，根本不打扰里面的核心业务。

6. Controller (控制器)
   大楼角色： 业务部门的“前台经理”。

核心作用： 路由分发。 经理本人不干脏活累活，他只负责看你办什么业务（根据 URL 路径和请求方法如 GET/POST），然后把对应的数据指派给具体的“业务专员”（Service）去干。

常见任务： \* 暴露 URL 接口（如 @Post('/login')）。

接收管道处理好的干净数据，传给对应的 Service。

7. Service (业务逻辑)
   大楼角色： 真正干活的“底层打工人/专家”。

核心作用： 核心业务逻辑。 这里是整个后端应用最值钱的地方，所有复杂的数学计算、去数据库（MySQL/Redis）增删改查数据的脏活累活，全部在这里完成。

常见任务： \* 拿着账号密码去数据库里比对。

调用第三方 API 发送短信验证码。

8. Interceptor (拦截器，响应后)
   大楼角色： 礼品包装部。

核心作用： 当 Service 把数据算完了，吐回出来时，拦截器会再次拦截它，对吐出的数据进行统一的格式化包装。

常见任务： \* 包装统一的 JSON 格式。比如 Service 只返回了 [ { name: "张三" } ]，拦截器会把它包装成大厂规范：{ code: 200, message: "success", data: [ { name: "张三" } ] }。

计算并打印出整个请求最终耗时了多少毫秒。

9. Exception Filter (异常过滤器)
   大楼角色： 公司的“公关部 / 危机处理小姐姐”。

核心作用： 如果有报错，出来擦屁股。 在上述 2 到 8 的任何一步中，如果代码突然抛出异常（比如数据库连不上了、代码写错空指针了、管道校验失败了），整个主流程会立刻中断，直接弹到“异常过滤器”这里。

常见任务： \* 拦截原本会暴露给用户的、丑陋且危险的服务器崩溃堆栈信息。

把它转化成对人类友好的报错提示（如 { code: 500, message: "服务器开小差了，请稍后再试" }）。

10. 响应客户端 (Client Response)
    大楼角色： 业务办完，把最终包装好的文件和礼品递给客户，客户高高兴兴回家。

11. 在你的活动平台中，RBAC（基于角色的权限控制）是如何结合 NestJS 实现的？
...
答案要点： 1. 异步队列：使用 Redis + BullMQ，将导出任务推入后台队列，不阻塞当前 HTTP 响应。2. 流式处理 (Stream)：避免将海量数据一次性加载到内存，而是从数据库 Cursor 边读边通过 Stream 写入 Excel（如 exceljs）。3. 多线程/多进程：如果是极其吃 CPU 的计算，可以启用 Node.js 的 Worker Threads，或者单独剥离出一个独立微服务来处理。
...
第二部分：数据库设计与高并发 (6-9) 6. 系统中的“活动 -> 子活动 -> 玩法 -> 任务”四级树状层级，在 MySQL 中你是如何设计的？
...
答案要点： 1. 数据库层面：利用外键的 ON DELETE CASCADE（级联删除），数据库自动清理。2. 业务代码层面（推荐）：使用数据库事务（Transaction）。为了保留历史记录，通常采用软删除（Soft Delete），在事务中依次将该活动及查出的所有下属节点状态置为 is_deleted = 1，保证强一致性。
...
第三部分：MySQL 索引与性能调优 (10-14) 10. 你的简历提到“解决多维搜索的性能问题”，你在使用 EXPLAIN 优化慢 SQL 时，主要关注哪几个核心字段？
...
第四部分：ORM (Prisma / TypeORM) 深度考察 (15-17) 15. 什么是 ORM 中的 N+1 查询问题？在处理“活动 -> 子活动”列表时，如何解决？
...
第五部分：Docker 与 PM2 线上部署 (18-20) 18. 既然已经用 Docker 做了容器隔离，为什么还要在容器内部使用 PM2 启动 Node 项目？直接 node main.js 不行吗？
...
21. 如何设计表1.必须有主键，且主键必须具备唯一性、不经常变动2.字段够用就行，不要字段冗余3.要考虑表的存储，做好分表4.做好默认值，避免null的时候索引失败5.敏感数据加密存储6.建立索引方便查数据（数据量大，并且搜索频繁的表需要建立索引， 需要用唯一值做索引，比如主键、userId等）

22. 如何设计接口1.路径统一，明确2.用 HTTP Method 明确（get、 post场景区分）3.接口一定要做版本控制4.统一响应格式
