...
# electron 如何处理进程间内存分配

# electron 如何做到热更新如何设计

维护一个更新文件
启动时将html、css、js下载到本地
再次启动修改loadFile的访问地址

# electron 如何做多端同构如何设计
...
如果你自己想开发一个类似的高性能原生模块（比如封装一个原生的地图 SDK、视频播放器、或者特殊的硬件读取组件），现在非常简单，不需要像以前那样写繁琐的 RCTBridgeModule 。

以下是现代 Expo Modules 开发标准流程：
Step 1: 脚手架初始化
使用 Expo 官方脚手架一键生成模块模板：

这会生成一个包含 src (JS), ios (Swift), android (Kotlin) 和 example (测试 App) 的完整目录结构。
Step 2: 编写原生代码 (iOS / Android)

- iOS ( ios/MyCustomModule.swift ) :
  使用 ExpoModulesCore 定义 Name("MyModule") ，并通过 Function("doSomething") { ... } 暴露方法，或使用 View(MyView.self) 暴露 UI 组件。
- Android ( android/.../MyCustomModule.kt ) :
  使用类似的 Kotlin DSL 定义相同的 Name , Function , View 。
  Step 3: 编写 JS / TS 包装层
  在 src/index.ts 中：
...
// 引入底层模块
const MyCustomModule = requireNativeModule
('MyCustomModule');

// 暴露给前端用户
export function doSomething(value: 
string) {
  return MyCustomModule.doSomething
  (value);
}
Step 4: 在 Example App 中边写边测
脚手架生成的 example 文件夹是一个完整的 Expo 项目，它的 package.json 已经把本地你的模块 link 进去了。
你可以在 example 目录下执行 npx expo run:ios ，一边修改 Swift/Kotlin 代码，一边在模拟器上看效果。
Step 5: 发布到 NPM
测试无误后，编译 TS 代码并发布：

npm run build
npm publish
总结 ：现代 React Native 模块开发得益于 Expo Modules API 和 JSI 技术 ，已经从过去的“黑盒异步桥接”变成了“像写普通函数一样编写跨语言代码”。 expo-image 就是这一范式理念的完美践行者。
...
# react-native 原生构建时能实现自动连接node_modules中原生代码的原因

ios： 就是podfile这个配置 autolinking， require File.join(File.dirname(`node --print "require.resolve('expo/package.json')"`), "scripts/autolinking")
安卓： 就是build.gradle文件中这个配置 ，react-native-gradle-plugin
dependencies {
...
# React Native 的Expo体系都包含哪些，给我梳理一下，并说一下对应的原理

1. Expo CLI & Metro Bundler
   是什么： 你敲 npx expo start 时启动的本地服务。

底层原理： Expo CLI 本质上包装了 React Native 底层的 Metro 打包器。Metro 就像前端的 Webpack/Vite，它的原理是监听你的 JS/TS 文件变化，实时编译成抽象语法树（AST），然后打包成一个 jsbundle 文件，并通过本地局域网的 WebSocket 实时推送到你的手机或模拟器上（这就是 Hot Module Replacement, HMR 热更新的原理）。

2. Expo Go
   是什么： 那个你能在 App Store 直接下载的蓝色沙盒 App。

底层原理： 这是一个提前编译好的“超级原生壳子”。里面包含了 React Native 引擎，以及 Expo 官方维护的所有原生模块（相机、蓝牙、文件系统等）的 C++/Java/Swift 代码。当你扫码时，它其实只是通过 HTTP 下载了你电脑上的 JS 代码跑在里面。

局限性： 因为壳子是固定的，所以一旦你引入了第三方包含自定义原生代码的库（比如支付宝 SDK），Expo Go 就装不下了，必须换用下面提到的定制壳子（Development Build）。

3. Expo Router
   是什么： 基于文件的路由系统（写法极其像 Next.js 的 app 目录）。

底层原理： 在它出现之前，RN 主要是用 react-navigation，需要你手动写一堆 <Stack.Screen> 配置表。Expo Router 在编译阶段，会自动扫描你 app/ 目录下的文件结构，然后在底层自动帮你生成 react-navigation 的配置代码，同时自动处理了 App 的 Deep Link（深度链接，比如在浏览器点击链接直接跳到 App 某个页面）。

二、 原生能力调度层（跨界造轮子的核心）
这是 Expo 真正称霸 RN 生态的杀手锏。它解决了以往 RN 开发最痛苦的“原生环境配置”问题。

1. 持续原生生成 (CNG - Continuous Native Generation) & Config Plugins (配置插件)
   是什么： 让你再也不用手动去改 build.gradle 或 Info.plist 的机制。

底层原理： Expo 提出“原生目录 (/android 和 /ios) 应该是临时的编译产物”。当你执行 npx expo prebuild 时，系统会读取 app.json。此时，Config Plugins（一段段 JS/TS 脚本） 会介入，它们利用正则表达式和 AST 解析，在内存中动态修改生成的 Java/Objective-C/XML 配置文件。用完即毁，保证你的项目永远纯净。

2. Expo Modules API
   是什么： 就是咱们之前聊过的，用极简的 Swift/Kotlin 语法写原生组件的脚手架。

底层原理： 深度结合 React Native 的 JSI (JavaScript Interface)。它在底层用 C++ 自动生成了 JS 与原生的内存共享映射。让 JS 可以像调用本地方法一样同步调用原生代码，直接干掉了老旧的异步 Bridge 序列化开销。

三、 EAS 云端大基建（DevOps & 生产发布）
EAS (Expo Application Services) 是 Expo 商业化的核心，也是目前 RN 团队进行持续集成的首选。

1. EAS Build (云端编译)
   是什么： 把打包这种脏活累活扔给云端。

底层原理： 当你敲 eas build 时，CLI 会打包你的 JS 源码和 app.json 发送到 Expo 云端服务器。服务器会根据你的配置，临时拉起一台干净的 Mac 或 Linux 虚拟机，执行 CNG 生成原生代码，然后调用 gradlew assemble 或 xcodebuild 进行真实的底层二进制编译，最后把 .apk / .ipa 扔回给你。

2. EAS Update (OTA 热更新)
   是什么： 不通过应用商店，直接修复线上 Bug。

底层原理： 打包正式 App 时，里面不仅有 JS Bundle，还藏了一个 Expo 的 Update 客户端 SDK。当用户打开 App 时，SDK 会静默向 Expo 服务器发起请求：“有没有比我当前版本更新的 JS Bundle？”如果有，就在后台下载替换。下次启动，用户看到的直接就是新代码（注意：只能更新 JS 代码，不能更新原生 C++/Java 代码）。

3. EAS Submit (一键提交审核)
   是什么： 自动把包传给苹果 App Store 和 Google Play。

底层原理： 借助开源的 Fastlane 工具链，利用你在云端配置好的 Apple ID / Google 开发者 API 密钥，直接通过官方开放平台接口上传二进制文件并触发机器审核，让你告别难用的 Xcode 甚至网页端操作。
...
安装 EAS CLI： 这是 Expo 官方最新的命令行工具。

Bash
npm install -g eas-cli
注册并登录 Expo： 在终端中登录你的 Expo 账号。

Bash
eas login
准备开发者账号（极其重要）：

iOS： 你必须花 $99/年购买 Apple Developer Program 账号。没有这个，即使是 Expo 云端也无法帮你生成正式包。

Android： 你需要注册 Google Play Developer 账号（一次性 $25）。
...
Bash
eas build:configure
这会在你项目根目录生成一个极其重要的 eas.json 文件。里面定义了不同环境（如 development, preview, production）的打包策略。

确认包名 (Bundle ID / Package Name)：
这就是咱们上一个问题提到的。确保你 app.json 里的 ios.bundleIdentifier 和 android.package 已经改成了你自己的独立标识（比如 com.yourcompany.appname）。

阶段 3：云端构建打正式包 (Build)
这里开始真正生成要提交给应用商店的二进制文件。

构建 Android 正式包：

Bash
eas build --platform android --profile production
底层动作： EAS 会自动帮你生成 Android Keystore（签名证书）并妥善保管在云端，最后吐出一个 .aab (Android App Bundle) 文件。

构建 iOS 正式包：

Bash
eas build --platform ios --profile production
底层动作： EAS 会提示你登录 Apple ID，自动帮你去苹果开发者后台创建 App ID、生成发布证书（Distribution Certificate）和描述文件（Provisioning Profile），极其省心。最后吐出一个 .ipa 文件。

阶段 4：提交审核 (Submit)
你可以自己把生成的 .ipa 或 .aab 下载下来，手动拖到 App Store Connect 或 Google Play Console。但 EAS 提供了更帅的自动化命令：

Bash
eas submit --platform all
EAS 会自动把刚才云端打好的包，直接通过 API 接口推送到苹果和谷歌的后台。你只需要去后台填写应用截图、介绍、隐私政策，然后点击“提交审核”即可。

整个流程跑通一次之后，你会发现最痛苦的其实是各种开发者账号的注册和证书管理，而 Expo 已经尽全力把构建这一步做成“傻瓜式”了。
...
- **答案：** Taro 抽象出了一层核心的**运行时（Taro Runtime）**。
  ...
  - 只要是基于 DOM 接口进行渲染的现代前端框架（像 React 调用的 `react-dom`，Vue 调用的运行时渲染器（runtime-dom）），都可以**无缝对接到这个模拟环境上**。
...
figma mcp、内部连接接口平台的mcp（方便测试）、react mcp

skill-crater skill、sequential skill（帮忙规划ai任务的执行步骤）、内部工作流skill
