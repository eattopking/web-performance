...
4. 在每个图层上绘制展示内容 - paint过程
5. 将多个图层合并, 按照合理的顺序将多个图层合并, 显示到浏览器上 - composite过程
6. 后续会出现, 重排, 重绘, 图层合并的情况,transform:translate就是只进行了重排和图层合并， 没有进行重绘，其实重绘 就是paint的时间算是这几个过程里面最长的了
