...
我们的浏览器是多进程工作方式， 浏览器有一个主进程， 控制、创建和销毁其他进程，比我们我们每开启一个tab页， 就是开启了一个renderer进程，浏览器采用多进程模式，隔绝了各个进程之间的联系，使进程之间不会相互影响，避免了因为一个tab崩溃导致其他tab也跟着崩溃的情况发生，renderer进程适合我们日常工作最为相关的进程， renderer进程中js执行和页面渲染等， renderer进程就是渲染进程， 因为在一个tab页中打开同源tab页面，如果不进行opener设置的话， 打开的同源页面会和触发页面打开的tab页面共用一个进程， 其他情况打开的tab页都是单独自己一个进程的, 浏览器还有network进程（网络进程， 只有一个network进程），还有插件进程，每种插件都有进程都会创建一个进程， 这个插件可以服务于多个tab页面，因为插件进程会为每个tab页面创建一个实例，GPU进程（也是只有一个， 这个进程是用于3D图形的加速）等
