...
5. 分词和倒排索引是两回事, 分词是将中文或者英文、数字拆分成单个词, 倒排索引是利用这单个词, 和多个内容的id进行关联, 这样通过一个关键字就可以关联多个内容id, 搜索出多条内容了, simple其实是做了一件事, 使用jieba进行分词,分词时在字典先去查看字典中有没有已经存在的词,有就按照存在的词进行分词，字典中没有的, 就找分词规则进行分词
...
准备搜索这个需要作为例子， 调研过程、遇到的问题、如果解决，最后的效果
