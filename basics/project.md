...
缺点: 需要部署服务, 不能打包到安装包中, 无法集成在 electron 中
...
已经跑通demo, 测试搜索性能, 比普通sqlite3查询快五分之三
...
3. electron 打的包默认都是64位的

4. simple fts5 node 插件也是只有x64位的, 和我们的包可以适配, windows 和mac的进行了测试, linux的还没有测试, simple 也是有x64版本的node fts5插件的

5. 切换用户的时候需要重新获取数据库连接实例

6. 全文搜索出来的时候id, 还需对之前的排序逻辑进行重构
...
最终选择了sqlite3结合simple fts5 插件解决搜索问题, 因为技术方案统一, 并且包括分词可以保证效率,搜索的速度是普通like搜索的三倍左右, 是最优的方案
...
5. 分词和倒排索引是两回事, 分词是将中文或者英文、数字拆分成单个词, 倒排索引是利用这单个词, 和多个内容的id进行关联, 这样通过一个关键字就可以关联多个内容id, 搜索出多条内容了, simple其实是做了两件事, 使用jieba进行分词, 然后建立倒排索引, 倒排索引需要单词词典,
单词词典包含了可能分的词的所有信息, 分词时在字典中词, 也会按照字典中信息去存储, 查询的时候可以通过字典提供的信息,快速定位单词, 快速查询到结果, 字典中没有的, 那就得自己去查了, 就影响速度了
...
6. js 基础知识 + es6
7. css 基础知识 看题
8. leecode 算法和一些基础的笔试题， 正则知识
9. http 请求 先看题在快速看书
10. 项目贡献
11. 我的优点，项目经历， 亮眼的地方要准备
12. 我的职业规划， 提高技术， 能负责一个方向
13. 我遇到的坑， 两方面， 一方便开发时遇到的坑， 一方面是带团队遇到的坑
14. 需要将说的自己都写出来， 反复更正说顺了
15. 具体的有实现一个观察者模式， 原来记录那些笔试题都需要整了
16. 我的诉求， 我希望进入到未来的公司， 能够有机会负责一个方向，在或者以后有机会可以带领一个团队， 希望我将来巩固的团队的氛围时欢乐，我一个容易被其他人的情绪影响的人， 这可能也是我的一个缺点， 所以如果团队氛围时欢快的， 我就很容易被带动， 我感觉在一个氛围好的团队中工作效率也是会大大提高的，我希望在工作中可以有机会做一些有挑战的工作，比如一些新的东西的尝试， 还有我也想问一下如果我加入咱们
    公司， 您期望我担任什么样的角色呢
17. 我的未来计划：
