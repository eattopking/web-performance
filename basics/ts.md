...
1. 元数据，利用元数据的能力实现依赖注入
2. 在类中通过public定义构造函数中的参数，并且给类添加一个依赖注入的装饰器函数，然后参数的的类型设置成另一个类，这个时候给类的实例中的增加了一个以参数名称命名的属性，这个属性的值就是另一个类的实例，这个就是依赖的注入，就是靠着Reflect.metadata实现的
