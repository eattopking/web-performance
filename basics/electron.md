...
使用这个加载动态连接库的功能实现了一个学术资源加速的功能，加载这个动态链接，然后调用对应的api就可以打开一个浏览器页面里面提供的网站，在对应网站中搜索的时候就可以挂一个代理，加速搜索
...
印象比较深刻的就是对有道云笔记的搜索功能重构，这个搜索功能是由前端来完成的，因为我们的搜索要支持离线搜索，所以这个搜素功能由前端完成，之前的搜索有比较多的问题， 一个是搜索不全，在一个搜索速度慢，因为原来的搜索只能搜到摘要，本地并没有存储全搜笔记的内容，还有之前的搜索逻辑只是从本地表中查到全部内容之后在通过filter过滤，这样的方式效率很低，在遇到笔记条数多的时候搜索起来时间很长

所以结合这两点诉求我就开始调研优化方案

1. 针对搜索不全的问题，因为我们是编辑器有特殊的内容结构，所以我首先了解了解析内容的方式，然后将全部的内容进行存储，用于搜

2. 针对搜索速度慢的问题，我思考了一下在前端实现过滤性能肯定是不行的， 所以我就转而寻找sql搜素的方案

首相想到的elasticSearch，因为这是后端首选的搜索方案，但是结合我们的项目不合适因为我们需要打包到桌面端中

然后又接着调研发现一个search-index的库，这个库可以进行倒排索引和分词，内部依赖的是leveldb数据库，我也开发了demo，整体效果我也比较满意，性能也可以，但是又和我们现在所用的sqlite数据库有点重复，所以又继续调研看看从sqlite本身有没有方式进行搜索，后来发现sqlite中有一个fts5虚拟表模块就是用来做全局搜索的，但是还缺少分词和倒排索引，这个fts5可以扩展插件，发现了一个和微信分词实现逻辑相同的fts5插件，用这个中方案又实现了一个demo，发现性能也是很好，所以最后综合选择了sqlite+ fts5的这种方式实现搜索， 最后的优化效果用户反馈也很好

### electron进程间通信这个在云笔记中记录