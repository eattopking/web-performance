...
2. 要不就是通过file://文件绝对路径，通过直接访问文件加载文件
...
302 重定向后将服务端端将真实请求的地址放在响应头的location字段上，浏览器用这个真实请求地址去请求
...
11. 协商缓存生效时返回304状态码，不使用时返回服务端真实状态码， 强缓存生效时返回缓存的请求响应时真实的状态码， 不一定是200
