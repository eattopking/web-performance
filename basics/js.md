...
* 闭包定义: 一个函数内存定义了一个子函数，子函数引用了父函数的作用域的变量这时候就是生成了闭包，如果这个时候有两个子函数一个引用了父函数的变量，一个没有引用，那么这个时候那个没有引用的也当作是一个闭包，这就是共享闭包
...
1. 全局变量不回被回收，let和const声明的全局变量不会作为window的属性，var声明的全局变量或者没有声明直接赋值的全局变量都会做为window的属性，
全局变量的内存只有页面关闭才会被回收
