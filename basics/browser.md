...
7. 浏览器在遇到script标签之后，会先渲染一遍之前解析好的html和css，这是为了因为在js中肯恩获取dom，所以要把之前的都渲染了，以备js中要获取，
如果没有遇到script的话那就等css和html都解析完毕然后最后在渲染
...
应用程序中通过window无法直接或者间接引用到的内存就是垃圾， 因为window上的属性的内存不会被垃圾回收机制回收
